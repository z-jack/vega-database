import * as AllDataFlow from "vega-dataflow";
...
export async function view2dataDot(view, stamp) {
  console.log(view);
  const rt = view._runtime;
  let ops = rt.nodes,
    keys = Object.keys(ops);
  ...
  [nodes, edges] = extractDataGraph([nodes, edges]);
  console.log(nodes, edges);

  const rootNode = nodes[0];
  if (rootNode) {
    const data = JSON.parse(JSON.stringify(rootNode.value.value));
    const removeChangeSet = view.changeset().remove(() => true);
    view.change(rootNode.data, removeChangeSet).run();
    await new Promise((res) => {
      setTimeout(() => {
        const _run = {};
        let timer = 0;
        Object.entries(AllDataFlow).forEach(([k, v]) => {
          if (v && v.prototype && v.prototype.run) {
            _run[k] = v.prototype.run;
        ...
        const changeset = view.changeset().insert(data);
        timer = Date.now();
        view.change(rootNode.data, changeset).run();
        setTimeout(() => {
          Object.entries(_run).forEach(([k, v]) => {
            AllDataFlow[k].prototype.run = v;
          });
          res();
        }, 0);
      }, 0);
